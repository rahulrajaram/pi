...
describe("AgentSession compaction model overrides", () => {
	...
	// Our fork rejects degenerate compaction summaries (summaryCharsFloorForTokens floor:
	// min(2000, 0.25 * summarizedTokens) chars) and clamps reserveTokens to 25% of the
	// context window (effectiveReserveTokens). Upstream's tests were written against stock
	// semantics; summaries below are sized to pass the floor for their scenarios, and
	// model-override reserveTokens values are set at/below the clamp cap so raw == clamped.
	const EXTENSION_SUMMARY = `compacted history ${"x".repeat(1084)}`;
	const BUILTIN_SUMMARY = `built-in summary ${"x".repeat(850)}`;
	const BIG_SUMMARY = `big model summary ${"x".repeat(2090)}`;
	const AUTH_SUMMARY = `summary ${"x".repeat(200)}`;
	afterEach(() => {
		vi.restoreAllMocks();
	...
	it.each(["manual", "pre-prompt", "post-run", "overflow"] as const)(
		...
		async (path) => {
			...
			const harness = await createHarness({
				...
				settings: {
					compaction: {
						...
						modelOverrides: { "faux/faux-1": { reserveTokens: 1000, keepRecentTokens: 150 } },
					},
				},
				extensionFactories: [
					(pi) => {
			...
			const recentUserId = seedHistory(harness, path === "pre-prompt" ? 3300 : 650);
			...
			if (path === "manual") {
				...
				harness.setResponses(
					path === "overflow"
						...
						: [fauxAssistantMessage(path === "post-run" ? "z".repeat(12000) : "done")],
				);
			...
			expect(harness.eventsOfType("compaction_end")[0]).toMatchObject({
				...
				result: { summary: EXTENSION_SUMMARY },
			});
	...
	it.each(["manual", "automatic"] as const)("passes resolved budgets to built-in %s summarization", async (path) => {
		...
		harnesses.push(harness);
		const recentUserId = seedHistory(harness, 3300);
		const budgets: Array<number | undefined> = [];
		harness.setResponses([
			(_context, options) => {
				budgets.push(options?.maxTokens);
				return fauxAssistantMessage(BUILTIN_SUMMARY);
			},
	...
	it("uses the newly selected model without changing ordinary settings", async () => {
		const harness = await createHarness({
			...
			settings: {
				compaction: {
					reserveTokens: 10,
					modelOverrides: { "faux/big": { reserveTokens: 2500, keepRecentTokens: 150 } },
				},
			},
			extensionFactories: [
				(pi) => {
		...
		expect(harness.eventsOfType("compaction_end")).toHaveLength(1);
		expect(harness.eventsOfType("compaction_end")[0]?.result?.summary).toBe(BIG_SUMMARY);
	...
	it("captures model identity before awaiting summarization auth", async () => {
		...
		harness.setResponses([
			(_context, options, _state, model) => {
				...
				return fauxAssistantMessage(AUTH_SUMMARY);
			},
		]);
		await harness.session.compact();
